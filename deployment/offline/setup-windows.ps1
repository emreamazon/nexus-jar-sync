...
function Install-SyncBatch([string]$InstallRoot,[string]$EnvironmentName){
    $path=Join-Path $InstallRoot 'sync-now.bat';$replace=$false
    if(Test-Path -LiteralPath $path){Write-Host "Existing manual sync launcher preserved: $path";$replace=Confirm-Step "Back up and replace sync-now.bat?";if(-not$replace){return}}
    $temporary=Join-Path $InstallRoot ".njs-sync-$([Guid]::NewGuid().ToString('N').Substring(0,8)).bat"
    $template=Join-Path $bundle 'tools\sync-now.bat.template';$content=(Get-Content -LiteralPath $template -Raw).Replace('__VENV_DIRECTORY__',$EnvironmentName)
    Set-Content -LiteralPath $temporary -Value $content -Encoding ASCII
    if($replace){$backup="$path.backup-$([DateTime]::UtcNow.ToString('yyyyMMddHHmmss'))";[IO.File]::Replace($temporary,$path,$backup);Write-Host "Previous launcher backed up: $backup"}else{Move-Item -LiteralPath $temporary -Destination $path}
    Write-Host "Manual one-shot launcher installed: $path";Write-SafeLog "manual_launcher=sync-now.bat result=installed"
}
...
try{
    ...
    Write-Phase "6 - Production preflight";Invoke-Checked $venvPython @('-m','nexus_jar_sync.main','--config',$config,'--dry-run') "production dry-run" $install;Write-Host "The next operation performs real production downloads into configured destinations.";if(-not(Confirm-Step "Run one active production synchronization?")){Write-Host "Setup complete. Double-click sync-now.bat for a future one-shot synchronization.";exit 0};Invoke-Checked $venvPython @('-m','nexus_jar_sync.main','--config',$config) "active production synchronization" $install;Write-Host "Production preflight succeeded. Application log: $logPath";Write-Host "Setup complete. Double-click sync-now.bat for each future one-shot synchronization.";exit 0
