...
For a guided per-user Windows deployment, extract the bundle beneath your user profile and double-click `setup-windows.bat`. It verifies the manifest plus the selected Python implementation, major/minor version, bitness, and normalized machine architecture before making changes. It preserves existing environments/configuration/data, reports credential-variable readiness without values, and separately gates test-download, test inspection, dry-run, and the first active synchronization. It never registers a task or service. Defaults are `%USERPROFILE%\NexusJarSync`, `venv-<version>`, `config\config.yaml`, `data`, `logs`, and `test-downloads`; the extracted bundle remains read-only source material. Run `setup-windows.bat -InstallationRoot "D:\Reviewed Path" -PythonExecutable "C:\Path To\python.exe" -SevenZipExecutable "D:\Approved Tools\7-Zip\7z.exe"` for explicit overrides. The wrapper does not bypass PowerShell execution policy.

After validation, setup creates `%USERPROFILE%\NexusJarSync\sync-now.bat`. Double-click it for exactly one active pass. It resolves the environment and config relative to itself, sets the installation directory as the working directory, reports success/failure, preserves the CLI exit code, and pauses for inspection. It never installs packages, changes configuration, loops, schedules another run, or deletes releases. Rerunning setup preserves an operator-modified launcher unless explicit backup-and-replacement is confirmed.
...
Inspect the summary, configured logs, state, and destination JARs. Windows operators normally continue with `sync-now.bat`. Linux operators may follow the systemd instructions. Windows Task Scheduler scripts remain optional advanced/legacy tools and are never invoked by guided setup.
